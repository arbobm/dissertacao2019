pdf:
	Rscript -e 'bookdown::render_book("index2.Rmd", output_format = "bookdown::pdf_book")'
	#rm -f *.log *.mtc* *.maf *.aux *.bcf *.lof *.lot *.out *.toc front-and-back-matter/abbreviations.aux
	Rscript -e 'browseURL("docs/_main.pdf")'

gitbook:
	Rscript -e 'bookdown::render_book("index2.Rmd", output_format = "bookdown::gitbook")'
	Rscript -e 'browseURL("docs/index.html")'

word:
	Rscript -e 'bookdown::render_book("index2.Rmd", output_format = "bookdown::word_document2")'
