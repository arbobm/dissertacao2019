...
As análises envolveram a área do bioma Pampa !!!IBGE, 2004!!!, que se estende por 176.476 km^2^ no estado do Rio Grande do Sul, sul do Brasil (Figura \@ref(fig:areaestudo)).
...
A silvicultura também constitui atividade em expansão no Rio Grande do Sul, com a plantação de eucalipto, acácia-negra e pinus [@Hasenack] !!!!!(Hasenack et al., dados não publicados) (tabela \@ref(tab:fisiocamp)).
...
```{r locbacias3ordem, fig.cap = "Localização das bacias de 3ª ordem utilizadas nas análises. CArb = Campo Arbustivo; CAre = Campo com Areais; CBDB = Campo com Barba-de-Bode; CEsp = Campo com Espinilho; CSR = Campo com Solos Rasos; CGra = Campo Graminoso; CMAC = Campo Misto com Andropogôneas e Compostas; CMCO = Campo Misto do Cristalino Oriental; FE = Floresta Estacional.", fig.height=12, fig.width=10, out.extra = ""}
```
...
\begin{table][h!]
    \centering
    \caption{Treze atributos funcionais indicadores de uso/ocupação do habitat e comportamento alimentar. Ver Figura \@ref(fig:medidas)
    para abreviações.]
